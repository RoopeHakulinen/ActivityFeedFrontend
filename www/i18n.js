app.config(function ($translateProvider) {
	$translateProvider.translations('en', {
		ABOUT_TITLE: 'Activer, Activer Oy',
		ABOUT_VERSION: 'Version',
		ACTIVER: 'Activer',
		BROWSE_ALL_ACTIVITY_TYPES: 'All activity types',
		BROWSE_INVITATION_SENT: 'Invitation sent',
		BROWSE_NO_ACTIVITIES: 'No activities found..',
		BROWSE_SWIPE_FOR_MORE: 'Swipe to browse activities.',
		COMMON_ACTIVITY: 'Activity',
		COMMON_BACK: 'Back',
		COMMON_CANCEL: 'Cancel',
		COMMON_CONNECTION_LOST: 'Connection lost',
		COMMON_CONNECTION_PROBLEM: 'No connection to Activer. Check your Internet access.',
		COMMON_EMAIL: 'Email',
		COMMON_FILL_ALL: 'Please fill in all the required fields',
		COMMON_LIST_FAILED: 'Fetching list failed. Please try again.',
		COMMON_NAME: 'Name',
		COMMON_OK: 'Okay',
		COMMON_PARTICIPANTS: 'participants',
		COMMON_SIGN_OUT: 'Sign out',
		COMMON_PASSWORD: 'Password',
		COMMON_UPDATE: 'Refresh',
		CREATE_ACTIVITY: 'Create an activity',
		CREATE_ACTIVITY_ACTIVITY_TYPE: 'Activity type',
		CREATE_ACTIVITY_CREATED: 'Activity successfully created',
		CREATE_ACTIVITY_FAILED: 'Activity creation failed. Please try again.',
		CREATE_ACTIVITY_LOCATION: 'Place',
		CREATE_ACTIVITY_LOCATION_HELP: 'Start to write to search',
		CREATE_ACTIVITY_LOCATION_NAME: 'Location name',
		CREATE_ACTIVITY_MAX_PARTICIPANTS: 'Max. participants',
		CREATE_ACTIVITY_MORE_INFORMATION: 'More information',
		CREATE_ACTIVITY_NO_ACTIVITY_TYPE: 'Choose activity type first',
		CREATE_ACTIVITY_NO_LOCATION: 'Choose location first',
		CREATE_ACTIVITY_REQUIRED_LEVEL: 'Level required (0.0 - 5.0)',
		CREATE_ACTIVITY_SEND_INVITATION_FOR_DIRECTS: 'Send invitation to Directs',
		CREATE_ACTIVITY_TIME: 'Time',
		DIRECT_ADD: 'Add as a Direct',
		DIRECT_ADD_FAILED: 'Adding Direct failed. Please try again.',
		DIRECT_ADDED: 'Direct added',
		DIRECT_REMOVE: 'Remove from Directs',
		DIRECT_REMOVE_FAILED: 'Removing Direct failed. Please try again.',
		DIRECT_REMOVED: 'Direct removed',
		EDIT_PROFILE_NAME_HINT: 'As John Doe',
		EDIT_PROFILE_UPDATED: 'Profile updated',
		FORGOT_PASSWORD: 'Forgot your password?',
		FORGOT_PASSWORD_SEND_NEW_PASSWORD: 'Send a new password',
		FORGOT_PASSWORD_DONE: 'Your password has now been reset, please check your email for new password.',
		FORGOT_PASSWORD_FAILED: 'Resetting password failed. Please check your email address.',
		INVITATION_ACCEPTED: 'Invitation accepted',
		INVITATION_DESC: 'would like to participate in',
		INVITATION_FAILED: 'Sending invitation failed. Please try again.',
		INVITATION_REJECTED: 'Invitation rejected',
		LOGIN_FAILED: 'Login failed, please check your username/password',
		LOGIN_OR_USE_ACTIVER: 'or use your Activer credentials',
		LOGIN_SIGN_IN: 'Sign in',
		LOGIN_SIGN_IN_WITH_FACEBOOK: 'Sign in with Facebook',
		LOGIN_SIGNING_IN: 'Signing in',
		MENU_ABOUT: 'About',
		MENU_CREATE_ACTIVITY: 'Create an activity',
		MENU_INVITATIONS: 'Invitations',
		MENU_MY_ACTIVITIES: 'My activities',
		MENU_PROFILE: 'Profile',
		MENU_SETTINGS: 'Settings',
		PROFILE_AGE: 'Age',
		PROFILE_PARTICIPATIONS: 'Participations',
		PROFILE_RATING: 'Rating (0-5)',
		PROFILE_RATING_COUNT: 'Ratings',
		SETTINGS_RANGE: 'Distance',
		SETTINGS_SIGN_OUT: 'Sign out from Activer',
		SHOW_ACTIVITY_COMMENT: 'Comment',
		SHOW_ACTIVITY_COMMENT_EMPTY: "Comment can't be empty",
		SHOW_ACTIVITY_COMMENTING_FAILED: 'Comment creation failed. Please try again.',
		SHOW_ACTIVITY_WRITE_A_COMMENT: 'Write a comment',
		SIGN_OUT_CONFIRMATION_TEXT: 'Are you sure you want to sign out?',
		REGISTER_FAILURE: 'Registering failed. Please try again.',
		REGISTER_FIRST_NAME: 'First name',
		REGISTER_LAST_NAME: 'Last name',
		REGISTER_PASSWORD_HINT: 'Password - 6 characters or more',
		REGISTER_REGISTER: 'Sign up',
		REGISTER_SUCCESS: 'Registering succeeded. Signing in.'
	});

	$translateProvider.translations('fi', {
		ABOUT_TITLE: 'Activer, Activer Oy',
		ABOUT_VERSION: 'Versio',
		ACTIVER: 'Activer',
		BROWSE_ALL_ACTIVITY_TYPES: 'Kaikki lajit',
		BROWSE_INVITATION_SENT: 'Kutsu lähetetty.',
		BROWSE_NO_ACTIVITIES: 'Ei aktiviteetteja..',
		BROWSE_SWIPE_FOR_MORE: 'Pyyhkäise ruutua selataksesi aktiviteetteja.',
		COMMON_ACTIVITY: 'Aktiviteetti',
		COMMON_BACK: 'Takaisin',
		COMMON_CANCEL: 'Peruuta',
		COMMON_CONNECTION_LOST: 'Yhteys katkesi',
		COMMON_CONNECTION_PROBLEM: 'Ei yhteyttä Activeriin. Tarkista, että olet yhteydessä verkkoon.',
		COMMON_EMAIL: 'Sähköposti',
		COMMON_FILL_ALL: 'Täytä kaikki vaaditut kohdat',
		COMMON_LIST_FAILED: 'Listan hakeminen epäonnistui. Yritä uudelleen.',
		COMMON_NAME: 'Nimi',
		COMMON_OK: 'Ok',
		COMMON_PARTICIPANTS: 'osallistujaa',
		COMMON_SIGN_OUT: 'Kirjaudu ulos',
		COMMON_PASSWORD: 'Salasana',
		COMMON_UPDATE: 'Päivitä',
		CREATE_ACTIVITY: 'Luo aktiviteetti',
		CREATE_ACTIVITY_ACTIVITY_TYPE: 'Aktiviteettityyppi',
		CREATE_ACTIVITY_CREATED: 'Aktiviteetti luotu',
		CREATE_ACTIVITY_FAILED: 'Aktiviteetin luonti epäonnistui. Yritä uudelleen.',
		CREATE_ACTIVITY_LOCATION: 'Paikka',
		CREATE_ACTIVITY_LOCATION_HELP: 'Aloita kirjoittamaan etsiäksesi',
		CREATE_ACTIVITY_LOCATION_NAME: 'Sijainnin nimi',
		CREATE_ACTIVITY_MAX_PARTICIPANTS: 'Maksimiosallistujamäärä',
		CREATE_ACTIVITY_MORE_INFORMATION: 'Lisätietoja',
		CREATE_ACTIVITY_NO_ACTIVITY_TYPE: 'Valitse ensin aktiviteettityyppi',
		CREATE_ACTIVITY_NO_LOCATION: 'Valitse ensin sijainti',
		CREATE_ACTIVITY_REQUIRED_LEVEL: 'Vaadittu taso (0.0 - 5.0)',
		CREATE_ACTIVITY_SEND_INVITATION_FOR_DIRECTS: 'Lähetä kutsu Directeille',
		CREATE_ACTIVITY_TIME: 'Aika',
		DIRECT_ADD: 'Lisää Directiksi',
		DIRECT_ADD_FAILED: 'Direktin luonti epäonnistui. Yritä uudelleen.',
		DIRECT_ADDED: 'Direct lisätty',
		DIRECT_REMOVE: 'Poista Directeistä',
		DIRECT_REMOVE_FAILED: 'Direktin poisto epäonnistui. Yritä uudelleen.',
		DIRECT_REMOVED: 'Direct poistettu',
		EDIT_PROFILE_NAME_HINT: 'Muodossa Etunimi Sukunimi',
		EDIT_PROFILE_UPDATED: 'Profiili päivitetty',
		FORGOT_PASSWORD: 'Unohditko salasanasi?',
		FORGOT_PASSWORD_SEND_NEW_PASSWORD: 'Lähetä uusi salasana',
		FORGOT_PASSWORD_DONE: 'Salasanasi on nyt nollattu, ja uusi salasana on lähetetty sähköpostiisi.',
		FORGOT_PASSWORD_FAILED: 'Salasanan nollaus epäonnistui. Varmista, että antamasi sähköpostiosoite oli oikein.',
		INVITATION_ACCEPTED: 'Pyyntö hyväksytty',
		INVITATION_DESC: 'haluaisi osallistua lajiin',
		INVITATION_FAILED: 'Pyynnön lähetys epäonnistui. Yritä uudelleen.',
		INVITATION_REJECTED: 'Pyyntö hylätty',
		LOGIN_FAILED: 'Kirjautuminen epäonnistui, tarkista käyttäjätunnus/salasana',
		LOGIN_OR_USE_ACTIVER: 'tai käytä Activer-tunnuksiasi',
		LOGIN_SIGN_IN: 'Kirjaudu',
		LOGIN_SIGN_IN_WITH_FACEBOOK: 'Kirjaudu Facebookin avulla',
		LOGIN_SIGNING_IN: 'Kirjaudutaan sisään',
		MENU_ABOUT: 'Tietoja',
		MENU_CREATE_ACTIVITY: 'Luo aktiviteetti',
		MENU_INVITATIONS: 'Kutsut',
		MENU_MY_ACTIVITIES: 'Minun aktiviteettini',
		MENU_PROFILE: 'Profiili',
		MENU_SETTINGS: 'Asetukset',
		PROFILE_AGE: 'Ikä',
		PROFILE_PARTICIPATIONS: 'Osallistumista',
		PROFILE_RATING: 'Palaute (0-5)',
		PROFILE_RATING_COUNT: 'Arvostelua',
		SETTINGS_RANGE: 'Etäisyys',
		SETTINGS_SIGN_OUT: 'Kirjaudu ulos Activerista',
		SHOW_ACTIVITY_COMMENT: 'Kommentoi',
		SHOW_ACTIVITY_COMMENT_EMPTY: "Kommentti ei voi olla tyhjä",
		SHOW_ACTIVITY_COMMENTING_FAILED: 'Kommentin lisäys epäonnistui. Yritä uudelleen.',
		SHOW_ACTIVITY_WRITE_A_COMMENT: 'Kirjoita kommentti',
		SIGN_OUT_CONFIRMATION_TEXT: 'Oletko varma, että haluat kirjautua ulos?',
		REGISTER_FAILURE: 'Rekisteröityminen epäonnistui. Yritä uudelleen.',
		REGISTER_FIRST_NAME: 'Etunimi',
		REGISTER_LAST_NAME: 'Sukunimi',
		REGISTER_PASSWORD_HINT: 'Salasana - Vähintään 6 merkkiä',
		REGISTER_REGISTER: 'Rekisteröidy',
		REGISTER_SUCCESS: 'Rekisteröityminen onnistui. Kirjaudutaan sisään.'
	});

	var preferredLanguage = localStorage.getItem("language");
	if (preferredLanguage === null)
	{
		preferredLanguage = 'fi';
	}
	$translateProvider.preferredLanguage(preferredLanguage);
});